    ...
    <body>
        <div class="header" style="min-height: 100vh;">
            ...
            <form action="uppro" method="post" enctype="multipart/form-data">
                <c:set value="${pro}" var="p"/>
                <div style="display: flex">
                    <!--begin of add-->
                    <div  style="width: 40%;margin-left: 10%;">
                        <table>
                            <tr><td>Name</td><td><input type="text" name="name" required="" value="${p.getName()}"/></td></tr>
                            <tr><td>Category</td><td>
                                    <select name="category" required="" style="width: 100%;">
                                        <option disabled="">Category</option>
                                        <c:forEach items="${category}" var="i">
                                            <option ${p.getCatID()==i.getID()?"selected":""} value="${i.getID()}">${i.getName()}</option>
                                        </c:forEach>
                                    </select> 
                                </td></tr>
                            <tr><td>Quantity</td><td><input required="" type="number" min="1" value="${p.getQuantity()}" name="quantity"/></td></tr>
                            <tr><td>Price</td><td><input required="" type="number" min='0.01' value="${p.getPrice()}" step="0.01" name="price"/></td></tr>
                            <tr><td>Description</td><td><textarea required="" name="description" style="width: 100%; height: 50vh; resize: none;">${p.getDescription()}</textarea></td></tr>
                        </table>
                    </div>
                    <!--end of add-->
                    <div style="width: 40%; margin-left: 5%;">
                        ...
                        <table style="margin-left: 10%;">
                            <tr>
                                <td style="text-align: right;">How many pictures?</td>
                                <td style="text-align: center;"><a style="width: 10%;" class="btn" href="uppro?id=${p.getID()}&count=${count==p.getImgs().size()?p.getImgs().size():count-1}">-</a>
                                    <input class="not" style="width: 30%; text-align: center" type="number" readonly value="${count>0 && count <10?count:1}" name="count"/>
                                    <a style="width: 10%;" class="btn" href="uppro?id=${p.getID()}&count=${count==9?9:count+1}">+</a></td>
                            </tr>
                            <c:forEach begin="0" end="${count-1}" var="i">
                                <c:set value="a${i}" var="x"/>
                                <tr>
                                    <td style="width: 10%;"><a style="text-align: center; ${1==p.getImgs().size()?"display: none":""}" href="delpic?id=${p.getID()}&name=${i<p.getImgs().size()?p.getImgs().get(i).getAddress():""}" class="btn">delete</a></td>
                                    <td><img style="margin-left: 35%;" name="picture${i}" id="${x}"
                                             src="${i<p.getImgs().size()?p.getImgs().get(i).getAddress():"images/1f.png"}"></td>
                                    <td ><input style="width: 70%;transform: translateX(-80%);" type="file" ${i<p.getImgs().size()?'disabled=""':""} accept="image/*" name="image${i}" id="file" onchange="loadFile(event, ${x}, this)" ></td>
                                    
                                </tr><input style="display: none;" required="" value="${i<p.getImgs().size()?p.getImgs().get(i).getAddress():""}" name="getPic${i}" id="data${i}" />
                            </c:forEach>
                                    <input style="display: none;" type="text" value="${count}" name="count">
                                    <input style="display: none;" type="text" value="${p.getID()}" name="id">
                        </table>
                    </div>
                </div>
                <h5 style="text-align: center;"><input type='submit' style="width: 20%; cursor: pointer;" class="btn" value='CHANGE'/></h5>
            </form> 
